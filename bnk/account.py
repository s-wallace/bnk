"""Account, Transaction and Value classes."""
...
class Account(object):
    ...
    def __init__(self, name, topen):
        """Create a new account with the specified opening date."""
    ...
    def to_csv(self, stream):
        ...
        for (n, i) in enumerate(items):
            if isinstance(i, Value):
                ...
                r = [tfmt.format(i.tstart), tfmt.format(i.tend), None,
                     i.amount]
            else:
    ...
    def is_open(self, t):
        ...
        if t < self._topen:
            return False
        # by 'open' I mean that there may be transactions,
    ...
    def _check_time(self, t):
        """Ensure a time window t=(t_start, t_end) is valid.
        ...
        if ts <= self._topen:
            raise ValueError((
                "{0}: Can't perform Account operations at or prior to "
                "to opening {1:%Y-%m-%d}").format(self.name, self._topen))

        if self._tclose is not None and tf > self._tclose:
            raise ValueError((
                "{0}: Can't perform Account operations "
                "after closing {1:%Y-%m-%d}").format(self.name, self._tclose))
    ...
    def mark_value(self, value):
        ...
        if value.t == self._topen and value.value == 0.0:
            _log.warning("Account marks a 0.0 balance on opening date. "
                         "This is depreciated")
            return

        if value.t == self._tclose and value.value == 0.0:
            _log.warning("Account marks a 0.0 balance on closing date. "
                         "This is depreciated")
            return
        ...
        if self._tclose and value.t == self._tclose:
            raise ValueError("Can't mark a value at close except "
                             "via set_closing")
            ...
            if value.t == val.t:
                if val.value == value.value:
                    ...
                    raise ValueError((
                        ...
                        "been valued at {2}").format(self.name, val.t,
                                                     val.value))
    ...
    def set_closing(self, t):
        ...
        for trn in self._transactions:
            if t < trn.tend:
                raise ValueError((
                    "Can't close an account before the last "
                    "transaction %s" % repr(trn)))
    ...
    def carrylast(self, todate):
        ...
        if v[1] == 'No Data' or v[1] == 'Carried':
            ...
            if todate < lastvalue.t:
                raise ValueError((
                    "Can't carry to specified date, it occurs "
    ...
    def get_performance(self, start, end, keys):
        ...
        keys['start balance'] = startvalue[0]
        keys['end balance'] = endvalue[0]
        ...
        keys['gain'] = (endvalue[0] - startvalue[0] -
    ...
    def get_irr(self, start, end):
        """Implicity calculate the interest earnings (loss) over a period.
        ...
        for t in self._transactions:
            ...
            if t.tstart > start and t.tstart <= end:
                assert t.tend <= end, \
                    "A transaction appears to cross a value mark"
        ...
        for (_, timing) in [('Long', longmoney_timing),
            ...
            assert sum(Decimal(d[1]) *
                       Decimal(1.0 + top / 100.0) ** Decimal(d[0])
            ...
            while top - bot > 0:
                ...
                result = 0 + \
                    sum(Decimal(d[1]) *
                        Decimal(1.0 + rate / 100.0) ** Decimal(d[0])
                        for d in timing)
                ...
                raise Exception("This shouldn't happen. bot:%f top:%f" % (bot,
                                                                          top))

        rates = [float(((Decimal(1.0 + r / 100.0) ** Decimal(365)) -
...
class Transaction(collections.namedtuple('_T', "tstart tend amount")):
    ...
    def add_to_account(self, account):
        """Add this transaction to the specified account."""
...
class Period(collections.namedtuple('_P', 'start end name')):
    """A named period with start and end dates."""
...
class NoValueAtStartDate(Exception):
    """Indicates the start of a period does not align with a value mark."""

    pass


class NoValueAtEndDate(Exception):
    """Indicates the end of a period does not align with a value mark."""
