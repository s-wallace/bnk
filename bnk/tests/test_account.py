...
class AccountTest(unittest.TestCase):
    """Test cases for bnk.account module."""

    def test_account_openclose(self):
        """Test account api open/close semantics."""

        # Can't have an opening date on Jan 1, 1AD
        self.assertRaises(ValueError, account.Account,
                          "test", dt.date(1, 1, 1))

        # Can't close an account before it's open
        a = account.Account("test", dt.date(2011, 12, 30))
        self.assertRaises(ValueError, a.set_closing,
                          dt.date(2010, 12, 30))
    ...
    def test_account_marksimple(self):
        """Verify account api value marking."""

        # Can't mark at open or close
        a = account.Account("test", dt.date(2011, 12, 30))
        self.assertRaises(ValueError, a.mark_value,
                          account.Value(dt.date(2011, 12, 30), 100))
    ...
    def test_account_transactionsimple(self):
        """Verify account api adding transactions."""
        a = account.Account("test", dt.date(2011, 12, 30))
        a.add_transaction(account.Transaction(dt.date(2012, 1, 1),
                                              dt.date(2012, 1, 31), 100))
        ...
        # Can't add a transaction starting at a mark
        # but can add one ending at a mark
        self.assertRaises(ValueError, a.add_transaction,
                          account.Transaction(dt.date(2014, 4, 1),
                                              dt.date(2012, 4, 5), 50))
        a.add_transaction(account.Transaction(dt.date(2012, 3, 1),
                                              dt.date(2012, 4, 1), 20))

        # Can close at transaction window end
        a.add_transaction(account.Transaction(dt.date(2012, 4, 2),
                                              dt.date(2012, 5, 1), 20))
        ...
        self.assertRaises(ValueError, a.add_transaction,
                          account.Transaction(dt.date(2012, 4, 20),
                                              dt.date(2012, 5, 2), 100))

    def test_account_performance_simple(self):
        """Verify account api performance metrics (except irr)."""

        a = account.Account("test", dt.date(2011, 12, 30))
        a.add_transaction(account.Transaction(dt.date(2012, 1, 5),
                                              dt.date(2012, 1, 31), 100))
        a.add_transaction(account.Transaction(dt.date(2012, 1, 1),
                                              dt.date(2012, 3, 30), 100))
        a.add_transaction(account.Transaction(dt.date(2012, 3, 30),
                                              dt.date(2012, 4, 20), 100))
        a.add_transaction(account.Transaction(dt.date(2012, 4, 30),
                                              dt.date(2012, 5, 31), 100))
        a.mark_value(account.Value(dt.date(2012, 5, 31), 500))
        performance = {}
        a.get_performance(None, None, performance)
    ...
    def test_parsesimple(self):
        """Verify account record parsing and metrics / full workflow."""
        ...
        accts = read_bnk_data(recstr)['Account']
        ...
        self.assertRaises(NonZeroSumError, read_bnk_data, brknrecstr)
        ...
        # parser doesn't hold state between read_bnk_data calls
        # 1. this is broken
        brkrecstr = """12-30-2001 open a
                       12-31-2001 open a"""
        self.assertRaises(ValueError, read_bnk_data, brkrecstr)
        # Using two calls to read_records should result in distinct accounts
        self.assertTrue(read_bnk_data, "12-30-2001 open a")
        self.assertTrue(read_bnk_data, "12-31-2001 open a")
        a = read_bnk_data('12-30-2001 open a')['Account']['a']
        b = read_bnk_data('12-31-2001 open a')['Account']['a']
        self.assertTrue(a != b)

    def test_parseonelines(self):
        """Verify new oneline syntax."""
        recstr = """12-30-2001 open a
                    01-01-1900 open Assets
    ...
    def test_as_csv(self):
        """Verify account exports to csv file."""
        recstr = """12-30-2001 open a
                    01-01-1900 open Assets
        ...
        # check that first...
        expectedcsv = (
            "12/30/2001,12/30/2001,0.0,,12/30/2001,0.0,0,12/30/2001,0.0"
            ",0\r\n"
            "12/31/2001,12/31/2001,,100.0,12/31/2001,100.0,0,12/31/2001,100.0"
            ",0\r\n"
            "12/31/2001,12/31/2001,100.0,,12/31/2001,,100.0,12/31/2001,"
            ",100.0\r\n"
            "12/31/2002,12/31/2002,200.0,,12/31/2002,200.0,0,12/31/2002,200.0"
            ",0\r\n")
        self.assertEqual(expectedcsv, csv.getvalue())

    def test_performance(self):
        """Verify account performance metrics."""
        data = read_bnk_data(recstrings.a3t3b3b)

        perf = {}
        acct_a = data['Account']['a']
        acct_a.get_performance(dt.date(2001, 12, 31), dt.date(2002, 12, 31),
                               perf)
        ...
        self.assertRaises(ValueError, acct_a.get_performance,
    ...
    def test_range(self):
        """Test the Range class; they're just tuples in disguise."""
